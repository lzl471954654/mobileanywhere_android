...
class ShareImageCommentsActivity : AppCompatActivity() {
        ...
        pick_image.setOnClickListener {
            imagePicker.startChooser(this, object : ImagePicker.Callback() {
                override fun onPickImage(imageUri: Uri?) {
                    this@ShareImageCommentsActivity.imageUri = imageUri
                    Glide.with(this@ShareImageCommentsActivity)
                            .load(imageUri)
                            .into(image_data)
                }
            })
        ...
        Luban.with(this)
                ...
                .setCompressListener(object : OnCompressListener {
                    override fun onSuccess(file: File?) {
                        runOnUiThread {
                            Toast.makeText(this@ShareImageCommentsActivity,"开始压缩图片",Toast.LENGTH_SHORT).show()
                            upload(file!!)
                        }
                    }
                    ...
                    override fun onError(e: Throwable?) {
                        runOnUiThread {
                            Toast.makeText(this@ShareImageCommentsActivity,"压缩出错",Toast.LENGTH_SHORT).show()
                        }
                    }
    ...
    fun upload(file: File) {
        doAsync {
            ...
            name = "mobileanywhere/" + dateformat.format(Date(System.currentTimeMillis())) +"/"+ name + "@" + System.currentTimeMillis() + ".jpg"
            sendToGitHub(name,base64, object : RequestCallback {
                ...
                override fun success(json: String?) {
                    ...
                    bogComment.blogCommentsMediaUrl = json
                    ...
                    RequestCenter.requestPost(RequestCenter.ADD_COMMENT,map, CallBackParser(object : RequestCallback {
                        override fun success(json: String?) {
                            runOnUiThread {
                                showSnackBar(feedback_ac_submit,json!!)
                                Toast.makeText(this@ShareImageCommentsActivity,"发送成功",Toast.LENGTH_LONG).show()
                            }
